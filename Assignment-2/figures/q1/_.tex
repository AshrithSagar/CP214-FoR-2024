...
\begin{tikzpicture}
  ...
  % Force F
  \draw[<-, thick] (-1,0.5) -- (0,0.5) node[midway, above] {\( \vec{\mathbf{F}} \)};
  \node[circle, fill, inner sep=0.75pt] at (0,0.5) {};

  % Spring Force
  \draw[->, thick] (1,0.5) -- (2.5,0.5) node[midway, above] {\( \mathbf{\vec{F}_{spring}} \)};
  \node[circle, fill, inner sep=0.75pt] at (1,0.5) {};

  % Normal reaction
  \draw[->, thick] (0.55,0) -- (0.55,1.5) node[above] {\( \mathbf{\vec{N}_A} \)};
  \node[circle, fill, inner sep=0.75pt] at (0.55,0) {};

  % Gravity
  \draw[->, thick] (0.5,0.5) -- (0.5,-0.75) node[below] {\( m\vec{\mathbf{g}} \)};
