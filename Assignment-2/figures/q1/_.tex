...
\begin{tikzpicture}
  ...
  % Spring Force
  \draw[->, thick] (1.2,0.5) -- (2.7,0.5) node[midway, above] {\( \mathbf{\vec{F}_{spring}} \)};
  \node[circle, fill, inner sep=0.75pt] at (1.2,0.5) {};

  % Normal reaction
  \draw[->, thick] (0.65,0) -- (0.65,1.4) node[above] {\( \mathbf{\vec{N}_B} \)};
  \node[circle, fill, inner sep=0.75pt] at (0.65,0) {};

  % Gravity
  \draw[->, thick] (0.6,0.5) -- (0.6,-0.7) node[below] {\( m\vec{\mathbf{g}} \)};
